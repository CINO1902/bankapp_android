...
class choosenetwork with ChangeNotifier {
  int _value = 0;
  int _group = 0;
  int _index = 0;
...
class choosefrequency with ChangeNotifier {
  int _value = 0;
  int _group = 0;
  int _index = 0;
...
class checklogged with ChangeNotifier {
  late String _token;
  bool _value = true;
  ...
  _loadfrompref() async {
    final _pref = await SharedPreferences.getInstance();
    _token = _pref.getString('showloggedprovider') ?? '';
    if (token == '') {
      _value = true;
    } else {
      _value = false;
  ...
  _savetopref() async {
    ...
    _pref.setString('showloggedprovider', _token);
  }
  // showindex(index1) {
  //   index = index1;
